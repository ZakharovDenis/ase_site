        ...
        if request.method == "POST":
            ...
            if user is not None:
                if user.is_active==0:
                    ...
                    return render(request, "ase_site/auth_core/templates/login_form.html", {"form": userform, 'values': ['Дождитесь активации']})
                else:
                    login(request,user)
                    if nxt != "":
                        return redirect(nxt)
                ...
                userform = UserLoginForm()
                return render(request, "ase_site/auth_core/templates/login_form.html", {"form": userform, 'values': ['Неверный логин или пароль']})
        else:
            userform = UserLoginForm()
